<template>
    <div>
        ...
        <table class="table table-bordered table-hover" v-if="list && list.length">
            <!-- liste doluysa ve uzunluğu 0'dan büyükse !-->
            <tr>
                <th>Id</th>
                <th>İsim</th>
                <th>Email</th>
            ...
            <tr v-for="{ id,name,email } in list">
                <!-- eğer column sayısı azsa içerikler bu şekilde de getirilebiliyor !-->
                <td>{{ id }}</td>
                <td>{{ name }}</td>
                <td>{{ email }}</td>
                <td>
...
export default {
    ...
    methods: {
        fetchData(page = 1) {
            ...
            this.list = null;
            axios.get("/users", {params: {page}}) // sadece users yazmayı bootstrap.js'teki baseURL sayesinde yaptık
                .then(response => {
        ...
        editData(id) {
            axios.get("/users/" + id) // sadece users yazmayı bootstrap.js'teki baseURL sayesinde yaptık
                .then(response => {
                    this.$refs.userModal.errorMessage = '';
        ...
        deleteData(id) {
            swal.fire({
                title: 'Emin misiniz ?', //sweet alert kişiselleştirme işlemleri
                ...
                if (result.value) { //eğer değer true geldiyse
                    axios.delete("/users/" + id)
                        .then(response => {
                            this.fetchData();
                            toastr.success('Kullanıcı Silindi','Kullanıcı');
                        })
                        .catch(error => {
                            if (error.response != null)
                                this.errorMessage = error.response.data.message;
